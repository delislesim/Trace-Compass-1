...
//        fBot.waitUntil(new ZoomCondition(timegraph));
        ...
        fBot.waitUntil(new ViewDirtyCondition());
        KEYBOARD.pressShortcut(Keystrokes.RIGHT);
...
//    @Test
    public void testToolBarFollowCPUForwardBackward() {
        ...
        /* set selection to trace start time */
        TmfSignalManager.dispatchSignal(new TmfSelectionRangeUpdatedSignal(this, START_TIME));
        fBot.waitUntil(new ViewIsReadyCondition(getTimeGraphView(), new TmfTimeRange(START_TIME, START_TIME), START_TIME));
        ...
        fViewBot.toolbarButton(FOLLOW_CPU_FORWARD).click();
        fBot.waitUntil(new ViewDirtyCondition());
        fBot.waitUntil(new ViewIsReadyCondition(getTimeGraphView(), new TmfTimeRange(TID2_TIME2, TID2_TIME2), TID2_TIME2));
...
//        fBot.waitUntil(new ViewDirtyCondition());
        fViewBot.toolbarButton(FOLLOW_CPU_FORWARD).click(SWT.SHIFT);
//        fBot.waitUntil(new ViewDirtyCondition());
        fBot.waitUntil(new ViewIsReadyCondition(getTimeGraphView(), new TmfTimeRange(TID2_TIME2, TID5_TIME1), TID5_TIME1));
...
//    }
    ...
    private class ViewIsReadyCondition extends DefaultCondition  {
        ...
        private ViewIsReadyCondition(AbstractTimeGraphView view, @NonNull TmfTimeRange selectionRange, @NonNull ITmfTimestamp visibleTime) {
            fView = view;
    ...
    private AbstractTimeGraphView getTimeGraphView() {
        IWorkbenchPart part = fViewBot.getViewReference().getPart(false);
        return (AbstractTimeGraphView) part;
    }
}
